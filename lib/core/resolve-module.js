...
function loadJsHyperclickExtensionsConfig(basedir) {
    const jsHyperclickJsonPath = findJsHyperclickJson(basedir);

    if (jsHyperclickJsonPath) {
        const jsHyperclickJson = require(jsHyperclickJsonPath)

        return jsHyperclickJson.extensions
    }
}
...
export default function resolveModule(
  ...
  if (aliasConf) {
    for (const key of Object.keys(aliasConf)) {
      ...
      if (regexp.test(moduleName)) {
          let absPath = moduleName.replace(regexp, `${aliasConf[key]}/`)

          moduleName = path.relative(basedir, absPath)
      }
    }
  }
  ...
  try {
      const absPath = path.join(basedir, moduleName)
      const extensions = loadJsHyperclickExtensionsConfig(basedir)

      if (extensions) {
          resolveOptions.extensions = extensions
      }

      if (fs.statSync(absPath).isDirectory() && extensions) {
          for (const ext of extensions) {
              const indexFilename = `index${ext}`

              if (fs.existsSync(path.join(absPath, indexFilename))) {
                  moduleName = `${moduleName}${path.sep}${indexFilename}`
                  break
              }
          }
      }
